...
const AppReactApexCharts = React.lazy(() => import('@/libs/styles/AppReactApexCharts'))

// Data for Doctor Statistics Tabs
const tabData = [
  {
    type: 'specialization',
    avatarIcon: 'tabler-briefcase',
    series: [
      { data: [20, 15, 30, 25, 18] } // Example data representing counts for different specializations
    ],
    categories: ['Cardiology', 'Neurology', 'Orthopedics', 'Pediatrics', 'Dermatology'] // Example categories
  },
  {
    type: 'availability',
    avatarIcon: 'tabler-calendar',
    series: [{ data: [10, 5] }], // Example data representing counts for available and unavailable doctors
    categories: ['Available', 'Unavailable'] // Example categories
  }
]
      ...
      label={
        ...
        >
          ...
          <Typography className='font-medium capitalize' color='text.primary'>
            {item.type === 'specialization' ? 'Specialization' : 'Availability'}
          </Typography>
        </div>
      }
    />
...
const DoctorStatisticsTabs = ({ serverMode }) => {
  // States
  const [value, setValue] = useState('specialization')
  ...
  const colors = Array(5).fill(rgbaToHex(`rgb(${theme.palette.primary.mainChannel} / 0.16)`))
  ...
  const options = {
    ...
    xaxis: {
      ...
      categories: tabData.find(tab => tab.type === value)?.categories || [],
      labels: {
        style: {
          colors: disabledText,
      ...
      <CardContent>
        <TabContext value={value}>
          <TabList
            ...
            aria-label='doctor statistics tabs'
...
export default DoctorStatisticsTabs
